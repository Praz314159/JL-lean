...
`κ = 2⁻¹²⁸`: `n_rp = 256`, `[α_rp, β_rp] = [√30, √337]` (ℓ₂-norm ratio), `b = 125`. Exposed as a
...
/-- The BS23 / RoKoko concrete parameters at `κ = 2⁻¹²⁸`: norm-ratio window `[√30, √337]`. -/
noncomputable def bs23Params : JL.Params :=
  { n := 256, α := Real.sqrt 30, β := Real.sqrt 337, b := 125 }
...
def concreteNormPreservation : Prop := JL.Lemma5_NormPreservation kappa128
